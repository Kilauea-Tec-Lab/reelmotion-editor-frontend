...
process.env.GOOGLE_APPLICATION_CREDENTIALS = process.env.GOOGLE_APPLICATION_CREDENTIALS || 'gcp-credentials.json';
process.env.GOOGLE_CLOUD_PROJECT = process.env.GOOGLE_CLOUD_PROJECT || process.env.GCP_PROJECT_ID || 'deft-processor-465219-e0';

const jwtPath = require.resolve('google-gax/node_modules/google-auth-library/build/src/auth/jwtclient.js');
...
JWT.prototype.fromJSON = function patchedFromJSON(json) {
	...
	if (json && typeof json === 'object') {
		...
		console.log('[debug] json.private_key starts:', typeof json.private_key === 'string' ? json.private_key.slice(0, 30) : null);
