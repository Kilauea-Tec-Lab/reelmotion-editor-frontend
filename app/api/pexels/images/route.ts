...
export async function GET(request: NextRequest) {
  ...
  const query = (searchParams.get("query") || "curated").trim();
  ...
  try {
    ...
    console.error("Pexels images proxy error:", error);
    return NextResponse.json({ error: "Failed to fetch Pexels images" }, { status: 500 });
  }
}
